...
import { buildDeptIndex, buildUploadPlan, executeUploadRows } from "./lib/upload-core.mjs";
...
const deptIndex = () => buildDeptIndex(mapMeta?.map?.sections);
...
// uploadPlan feeds the dry-run plan block and the final report. The plan data
// comes from the shared core; printing stays here (CLI presentation).
const { uploadPlan, missingGroups } = buildUploadPlan({ people, targetGroupOf, resolveGroup, TO });
if (TO) {
...
// Row execution: shared core (uploader/lib/upload-core.mjs) — same mutation
// logic the Job/UI binding runs. CLI stays the adapter: page lifecycle,
// callbacks, and reports live here.
const page = await context.newPage();
const collected = [];
let results;
try {
  results = await executeUploadRows({
    page,
    people,
    TO,
    SAVE,
    ...
    onRow: (rec) => __pushRow(collected, rec),
    onArtifact: ({ absPath, relPath, kind, url }) => __emitArtifact(absPath, relPath, kind, url),
  });
} finally {
